...
/**
 * This class takes care of indexing, it retrieves the list of
 * FacebookUsers from the database reads the feed of the users 
 * and computes the number of links between each one of the users
 * 
 * @author Kervin Ramen
 * 
 */
public class IndexEngine {
    ...
    private void updateIndex(NamedFacebookType to, NamedFacebookType from, Date commentDate) {
        ...
        if (!to.getId().equals(from.getId())) {
            UserIndex userIndex = UserIndex.searchUserIndex(Integer.parseInt(to.getId()), Integer.parseInt(from.getId()));
